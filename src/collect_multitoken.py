...
class ContextsDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, i2w, sentences, context_size, tokenizer, sampling_probs, n_sentences=None):
        ...
        # todo: keep a dictionary of how often a word has been sampled
...
def main():
    ...
    ids2w = {}
    for t in targets:
        ids1 = tuple(tokenizer.encode(t, add_special_tokens=False))
        ids2 = tuple(tokenizer.encode(' {}'.format(t), add_special_tokens=False))
        ids3 = tuple(tokenizer.encode(str(t.title()), add_special_tokens=False))
        ids4 = tuple(tokenizer.encode(' {}'.format(t.title()), add_special_tokens=False))

        targets_ids[t] = []
        for ids_tuple in {ids1, ids2, ids3, ids4}:
            targets_ids[t].append(ids_tuple)
            ids2w[ids_tuple] = t

    max_tok_in_word = max([len(toks) for toks in ids2w.keys()])
    ...
    target_counter = {target: 0 for target in targets_ids}
    for sentence in sentences:
        relevant_ids = []
        for w in sentence:
            if w in targets_ids:
                for ids_tuple in targets_ids[w]:
                    relevant_ids.append(ids_tuple)

        encoded = tokenizer.encode(' '.join(sentence), add_special_tokens=False)
        max_len = max([len(t) for t in relevant_ids])
        for i in range(len(encoded)):
            for _len in range(1, max_len + 1):
                if tuple(encoded[i:i + _len]) in relevant_ids:
                    target_counter[ids2w[tuple(encoded[i:i + _len])]] += 1
